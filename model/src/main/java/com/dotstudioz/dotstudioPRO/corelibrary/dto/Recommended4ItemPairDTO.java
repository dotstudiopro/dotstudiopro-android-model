package com.dotstudioz.dotstudioPRO.corelibrary.dto;

/**
 * Created by user on 15-06-2016.
 */
public class Recommended4ItemPairDTO {
    private RecommendedItemDTO recommendedItemDTO1;
    private RecommendedItemDTO recommendedItemDTO2;
    private RecommendedItemDTO recommendedItemDTO3;
    private RecommendedItemDTO recommendedItemDTO4;

    public RecommendedItemDTO getRecommendedItemDTO1() {
        return recommendedItemDTO1;
    }

    public void setRecommendedItemDTO1(RecommendedItemDTO recommendedItemDTO1) {
        this.recommendedItemDTO1 = recommendedItemDTO1;
    }

    public RecommendedItemDTO getRecommendedItemDTO2() {
        return recommendedItemDTO2;
    }

    public void setRecommendedItemDTO2(RecommendedItemDTO recommendedItemDTO2) {
        this.recommendedItemDTO2 = recommendedItemDTO2;
    }

    public RecommendedItemDTO getRecommendedItemDTO3() {
        return recommendedItemDTO3;
    }

    public void setRecommendedItemDTO3(RecommendedItemDTO recommendedItemDTO3) {
        this.recommendedItemDTO3 = recommendedItemDTO3;
    }

    public RecommendedItemDTO getRecommendedItemDTO4() {
        return recommendedItemDTO4;
    }

    public void setRecommendedItemDTO4(RecommendedItemDTO recommendedItemDTO4) {
        this.recommendedItemDTO4 = recommendedItemDTO4;
    }
}
